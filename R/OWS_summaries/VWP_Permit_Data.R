...
vwp_permits <- read.csv('https://deq1.bse.vt.edu/d.dh/ows-list-permits-export-vwp') #note permit writer field not getting pulled into R
facility_mp_fracs <- read.csv('https://deq1.bse.vt.edu/d.dh/facility_mp_frac_value_export?bundle%5B0%5D=intake&hydroid=&propcode_op=%3D&propcode=&fstatus=All&propname_op=%3D&propname=wd_current_mgy')
...
### JOIN FACILITY SUMMED facility_use_fractions TO VWPS DATAFRAME
...
for (i in 1:length(years)){
  ...
  # map_exports <- read.csv(paste('https://deq1.bse.vt.edu/d.dh/ows-awrr-map-export/wd_mgy?ftype_op=%3D&ftype=&tstime_op=between&tstime%5Bvalue%5D=&tstime%5Bmin%5D=',year,'-01-01&tstime%5Bmax%5D=',year,'-12-31&bundle%5B0%5D=intake&dh_link_admin_reg_issuer_target_id%5B0%5D=448933&dh_link_admin_reg_issuer_target_id%5B1%5D=379560&dh_link_admin_reg_issuer_target_id%5B2%5D=65668&dh_link_admin_reg_issuer_target_id%5B3%5D=90549&dh_link_admin_reg_issuer_target_id%5B4%5D=90550&dh_link_admin_reg_issuer_target_id%5B5%5D=92517&dh_link_admin_reg_issuer_target_id%5B6%5D=91200&dh_link_admin_reg_issuer_target_id%5B7%5D=77498&dh_link_admin_reg_issuer_target_id%5B8%5D=170026',sep=''))
                                 
  #RETRIEVE DATA FOR VWP ISSUING AUTHORITY ONLY
  map_exports <- read.csv(paste('https://deq1.bse.vt.edu/d.dh/ows-awrr-map-export/wd_mgy?ftype_op=%3D&ftype=&tstime_op=between&tstime%5Bvalue%5D=&tstime%5Bmin%5D=',year,'-01-01&tstime%5Bmax%5D=',year,'-12-31&bundle%5B0%5D=intake&dh_link_admin_reg_issuer_target_id%5B0%5D=91200',sep=''))
...
write.csv(vwp_permits,paste(savepath,"vwp_permit_data_","2014-2018_",gsub(":", "", gsub(" ", "-", Sys.time())),".csv",sep=''), row.names = FALSE)
