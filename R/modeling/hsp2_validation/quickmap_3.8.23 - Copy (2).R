...
################################################################################
################################################################################
# polygons_layer <- sp::SpatialPolygonsDataFrame(readWKT(watersheds$geom),data=watersheds[,1:ncol(watersheds)-1])
# 
# ps <- lapply(watersheds, Polygon)
# 
# # add id variable
# p1 <- lapply(seq_along(ps), function(i) Polygons(list(ps[[i]]), ID = names(watersheds)[i]  ))
# 
# # create SpatialPolygons object
# my_spatial_polys <- SpatialPolygons(p1, proj4string = CRS("+proj=longlat +datum=WGS84") )
# 
# pols <- FRK::df_to_SpatialPolygons(watersheds,keys="featureid",coords="geom",CRS())
# 
# # sp_layer <- sp::SpatialPolygonsDataFrame(readWKT(watersheds$geom))
# TEST = SpatialPointsDataFrame(watersheds$geom, data=watersheds[,1:ncol(watersheds)-1])
