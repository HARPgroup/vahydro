...
################
colorkey = list(right = list(args = list(key = list(labels = c("0x")))))



s <- sp::spplot(polygons, zcol=plotvar,
                col.regions=gray(rev(seq(0,1,.01))),
                main=paste0(plotname,"\n",segswhere),
                sub=c(paste0("Scenario A: ",scenario_a$ftype,", ",scenario_a$model_version,", ",scenario_a$runid, " (",length(na.omit(watersheds$scenario_b_Qout)),")\n",
                             "Scenario B: ",scenario_b$ftype,", ",scenario_b$model_version,", ",scenario_b$runid, " (",length(na.omit(watersheds$scenario_a_Qout)),")")),
                par.settings=list(fontsize=list(text=40)),
                ylab="test", colorkey = list(space = "left", height = 0.4)
)
...
spplot(polygons, zcol=plotvar,
       col.regions=gray(rev(seq(0,1,.01))),
       legend = list(inside = legendArgs, "nam"))

###################

# legend("topleft", legend=c(paste0(scenario_a$ftype,", ",scenario_a$model_version,", ",scenario_a$runid, " (",length(na.omit(watersheds$scenario_b_Qout)),")"), 
#                            paste0(scenario_b$ftype,", ",scenario_b$model_version,", ",scenario_b$runid, " (",length(na.omit(watersheds$scenario_a_Qout)),")")), 
#        fill = c("white","blue"), cex=3.5)

# length(na.omit(watersheds$scenario_b_Qout))
