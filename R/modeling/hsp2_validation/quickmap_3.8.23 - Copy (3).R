...
# rsegs_layer_sp <- sp::SpatialPolygonsDataFrame(readWKT(watershed_feature$geom[1]), data=watersheds[1,1:ncol(watersheds)-1])
...
################################################################################
################################################################################
filename <- paste0("TEST3.png")
png(file=paste(export_path,filename,sep=""), width=1500, height=1500)
# plot(rsegs_layer_sp, col = rsegs_layer_sp$Qout,
#      axes = 1, 
