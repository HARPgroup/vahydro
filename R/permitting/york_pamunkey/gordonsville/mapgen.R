...
mapgen <- function(start_point = data.frame(lat = 38.123193596633, lon = -78.203527325477, label = "Intake"),
                   points = data.frame(lat=double(),lon=double(),label=character()),
                   segswhere = "hydrocode IN ('vahydrosw_wshed_YP3_6470_6690', 'vahydrosw_wshed_YP3_6471_6470', 'vahydrosw_wshed_YP3_6472_6471')") {
  ...
  domain <- st_buffer(st_as_sfc(st_bbox(start_point_layer)), .005)
  ...
  ######################################################################
  # source("C:/Users/nrf46657/Desktop/GitHub/hydro-tools/GIS_functions/model_geoprocessor.R")
  source("C:/Users/nrf46657/Desktop/GitHub/vahydro/R/permitting/gordonsville/model_geoprocessor.R")
  # process rseg layer
  ...
  map_gg <- toner_map + 
    ...
    geom_label_repel(data = start_point_labels, aes(X, Y, group = 1, label = NAME), size = 10, fill="white", box.padding =1) +
    geom_label_repel(data = points_labels, aes(X, Y, group = 1, label = NAME), size = 10, fill="white", box.padding =1) +
...
# outfall: 38.123478, -78.202625
# facility: 38.124715996282, -78.202785065245


points = data.frame(lat=c(38.123478, 38.124715996282),
                    lon=c(-78.202625,-78.202785065245),
                    label=c("Outfall 001","Gordonsville Power Station")
                    )

# points = data.frame(lat=c(37.234062, 37.4144),
#                     lon=c(-80.178434,-79.9338),
#                     label=c("WVWA Spring Hollow Reservoir\nRoanoke River Intake",
#                             "WVWA Carvins Cove Reservoir\nTinker Creek Intake")
# )

map_gg <- mapgen(start_point = data.frame(lat = 38.123193596633, lon = -78.203527325477, label = "Quarry Intake"),
                 points = points, segswhere = "hydrocode IN ('vahydrosw_wshed_YP3_6470_6690', 'vahydrosw_wshed_YP3_6471_6470', 'vahydrosw_wshed_YP3_6472_6471')")

fpath = "C:/Users/nrf46657/Desktop/VWP Modeling/vwp - Gordonsville Power Station/"
fname = paste(fpath,"gordonsville_map.png",sep="")
ggsave(
  filename = fname,
  plot = map_gg,
  width = 20,
