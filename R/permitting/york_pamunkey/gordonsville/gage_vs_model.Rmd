...
model_flows <- zoo(as.numeric(as.character( model_data$Qout )), order.by = index(model_data) );
...
#####################################################
#qa 
model_flows_df <- data.frame(model_flows)
gage_flows_df <- data.frame(gage_flows)
length(model_flows_df[,1])
length(gage_flows_df[,1])

model_flows_df$model_date <- row.names(model_flows_df)
gage_flows_df$gage_date <- row.names(gage_flows_df)

dat_query <- "SELECT * 
              FROM model_flows_df
              LEFT JOIN gage_flows_df ON model_flows_df.model_date = gage_flows_df.gage_date"
dat_join <- sqldf(dat_query)
rownames(dat_join) <- as.Date(dat_join$model_date)
# model_flows <- dat_join$model_flows
# gage_flows <- dat_join$gage_flows
# 
# length(model_flows)
# length(gage_flows)

model_flows <- zoo(as.numeric(as.character(dat_join$model_flows)), order.by = index(dat_join));
gage_flows <- zoo(as.numeric(as.character(dat_join$gage_flows)), order.by = index(dat_join));

test <- zoo(as.numeric(dat_join[,1]))

framedates = as.Date(dat_join$model_date, format="%Y-%m-%d")
n=ncol(dat_join)
# zoodata = zoo(dat_join[,2:n], order.by=framedates)
model_flows_zoo = zoo(dat_join$model_flows, order.by=framedates)
gage_flows_zoo = zoo(dat_join$gage_flows, order.by=framedates)
#####################################################
# compare means
# mean(model_flows)
# mean(gage_flows)
...
model_loflows <- IHA::group2(model_flows_zoo)
gage_loflows <- IHA::group2(gage_flows_zoo)
...
# cmp_l90$USGS[!is.na(cmp_l90$USGS) & !is.infinite(cmp_l90$USGS)]
# cmp_l90$Model[!is.na(cmp_l90$Model) & !is.infinite(cmp_l90$Model)]
ymax_val <- max(c(cmp_l90$USGS[!is.na(cmp_l90$USGS) & !is.infinite(cmp_l90$USGS)],cmp_l90$Model[!is.na(cmp_l90$Model) & !is.infinite(cmp_l90$Model)]))
