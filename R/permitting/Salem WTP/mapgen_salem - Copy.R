...
RSeg_layer_geom <- geom_sf(data = RSeg_layer_sf,aes(geometry = geom),fill = 'goldenrod1',
                           color = 'gray30',lwd=2,alpha=0.25, inherit.aes = FALSE, 
                           show.legend = "line")
...
minorbasin_map <- basemap.obj + 
  ...
  scale_fill_manual(name = "Groundwater Withdrawal (MGD)",
                    values = c("#FFFF80"),
                    labels = c("white")
  ) +
  ...
  #                st.bottom=FALSE,st.size = 3.5, st.dist = 0.0285,anchor = c(x = -82.735,y = 36.7520))

  ggsn::scalebar(bb.gg, location = 'bottomleft', dist = 2, dist_unit = 'mi',transform = TRUE, model = 'WGS84',
                 st.bottom=FALSE,st.size = 3.5, st.dist = 0.0285,anchor = c(x = extent$x[1]+0.065,y = extent$y[1]+0.002))
  
  #x = extent$x[1],y = extent$y[1]
######################################################################################################
