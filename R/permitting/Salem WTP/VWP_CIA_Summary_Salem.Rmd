...
if (isFALSE(additional_rseg_ids) == FALSE  & length(additional_rseg_ids) > 0) {
    ...
    metric_df <- read.csv(paste(site,"/model-summary-users-export/",
                           rseg.hydroid,",",gsub(" ","",toString(additional_rseg_ids)),"/",runid,"/",users_metric,sep=""))
} else {
...
users_sql <- paste('SELECT usersdf.polygon_name AS "Sub-Watershed",
...
users <- width(users, j = 3, width = 0.65)
users <- width(users, j = 4, width = 1.75)
users <- width(users, j = 5, width = 0.95)
users <- width(users, j = 6, width = 1.75)
users <- width(users, j = 7, width = 1.2)
users <- width(users, j = 8, width = 0.95)
users <- width(users, j = 9, width = 1.0)
