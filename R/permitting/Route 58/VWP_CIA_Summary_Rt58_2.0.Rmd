...
author: ""
title: "VWP CIA Summary - Route 58 Lovers Leap"
output: 
...
params: 
  ...
  runid.list: [ "runid_600" ]
  fac.metric.list: [ "base_demand_mgy", "wd_mgy", "unmet_demand_mgy","base_demand_mgd", "wd_mgd","unmet30_mgd" ]
  rseg.metric.list: [ "Qout","l30_Qout",
...
Project:  Route 58 Lovers Leap VWP Permit application 21-0379  
Applicant:  Branch Civil, Inc.; Attn: Jeff Humphreys, Jr.

Project will require water during construction for dust suppression, soil amendment to meet optimum soil moisture content for compaction design standards, washing of equipment to keep sediment from being tracked onto roadways, irrigation of temporary and permanent vegetation stabilization, along with other miscellaneous construction activities. The applicant proposes to use temporary intakes at three locations in the vicinity of the road construction. All three intakes could operate at the same time, but cumulatively would not exceed the proposed maximum daily withdrawal.

This withdrawal is temporary and will only be required during road construction which is expected to last up to 5 years.

Proposed Withdrawal: 

* **Maximum instantaneous withdrawal** = 0.22 fps
* **Average daily withdrawal** = 0.035 mgd
* **Daily withdrawal limit** = 0.10 mgd 
* **Monthly withdrawal limit** = 3.1 mgd
* **Annual withdrawal limit** = 36.5 mg/yr (0.10 mgd)

<br>  
Withdrawal assumed to be 100% consumptive.
...
# Estimated Stream Flows: (Max Pumping - All Intakes) 
...
statsdf_sql <- paste('SELECT
                  CASE
                    ...
                    WHEN "Desc" = "base_demand_mgy" THEN "Requested Demand (mgy)"
                    WHEN "Desc" = "wd_mgy" THEN "Withdrawal Met (mgy)"
...
# Unmet Demand Plots:
Unmet demand grids help visualize unmet demand data as magnitudes of color intensity. The following depict the number of days with unmet demands for each month of the simulation (due to demands exceeding allowable withdrawal at the intake based on the cumulative conditions in the watershed and the flow-by rules in effect). The grid cells show the amount of unmet demand for each month [Number of Unmet Days & Amount (mgd)]. Hydrographs are shown for the period of the simulation with greatest unmet demand.
...
for (i in 1:length(runid.list)){
  ...
  if (!(is.null(fig_prop))) {
    fig_path <- fig_prop$code
    # we have an image, show it 
    cat("\n\n## Unmet Demand Grid: ",short_names[[runid.i]],"\n\n")
    cat(paste0("![](",fig_path,")"),"\n")
  }
...
Estimates for richness change are presented both as an absolute number of species (Richness Change (abs)) and as a percentage of the total number of species present (Richness Change (%)). Richness change calculations are derived from the estimated percent total consumptive use (For additional details on “elfgen” methodology, see https://onlinelibrary.wiley.com/doi/full/10.1111/1752-1688.12876).
