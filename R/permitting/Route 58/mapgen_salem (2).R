...
export_path <- paste(github_location,"/vahydro/R/permitting/Salem WTP/",sep="")
...
# extent = data.frame(x = c(-80.195, -79.945),
#                     y = c(37.165, 37.415))

extent = data.frame(x = c(-80.205, -79.935),
                    y = c(37.155, 37.425))
...
minorbasin_map <- basemap.obj + 
  ...
  scale_colour_manual(name = c("Legend"),
                      values = c("gray30","black","orange","gray30","blue"),
                      labels = c("River Segments","Intake","Wells","Transfer","Streamgage"),
                      guide = guide_legend(override.aes = list(linetype = c("solid", "blank", "blank","dotted","blank"), 
                                                               alpha = c(1,1,1,1,1),
                                                               shape = c(NA,16,16,NA,17)))) +
