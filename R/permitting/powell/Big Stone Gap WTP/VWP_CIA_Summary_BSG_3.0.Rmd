...
author: ""
title: "VWP CIA Summary - Big Stone Gap WTP"
output: 
  officedown::rdocx_document:
    ...
    page_margins:
      ...
      footer: 0.0
params: 
  rseg.hydroid: 477140 # Ex: Crooked Run = 476998, SF Powell below BSG = 477140, SF Powell at dam: 462757
  ...
  intake_stats_varname: "Qnatural"
---

```{r setup, include=FALSE}
#"runid_400","runid_600","runid_6001"
#"runid_4011","runid_6011","runid_6012"
...
```{r BigCherryInputs, include=FALSE}
BigCherry.rseg.hydroid <- 462757
BigCherry.rseg.metric.list <- c("remaining_days_p0")

#Big Cherry Reservoir RSEG MODEL INFO
print(paste("searching", site,"for river segment id", BigCherry.rseg.hydroid))
BigCherry.rseg.model <- om_get_model(site, BigCherry.rseg.hydroid)
BigCherry.rseg.elid <- om_get_prop(site, BigCherry.rseg.model$pid, entity_type = 'dh_properties','om_element_connection')$propvalue

if (exists("json_obj_url")) {
  BigCherry.rseg_obj_url <- paste(json_obj_url, BigCherry.rseg.model$pid, sep="/")
  BigCherry.rseg_model_info <- om_auth_read(BigCherry.rseg_obj_url, token,  "text/json", "")
  BigCherry.rseg_model_info <- fromJSON(BigCherry.rseg_model_info)
  BigCherry.rseg_report_info = find_name(BigCherry.rseg_model_info, "reports")
} else {
  message("Error: json_obj_url is undefined.  Can not retrieve model and scenario information. (Hint: Use config.R to set json_obj_url) ")
  BigCherry.rseg_model_info <- list()
}  

BigCherry.rseg_default_info = list(
  model_overview = list(
    "value" = "River segment model overview not provided."
  )
)

BigCherry.rseg_report_info <- merge.list(BigCherry.rseg_report_info, BigCherry.rseg_default_info)   

BigCherry.rseg_runs = FALSE
for (i in 1:length(runid.list)){
  runid.i <- runid.list[i]
  run.i <- sub("runid_", "", runid.i)

  BigCherry.rseg_run <- om_get_prop(site,BigCherry.rseg.model$pid, "dh_properties", runid.i)
  if (is.logical(BigCherry.rseg_runs)) {
    BigCherry.rseg_runs <- as.data.frame(BigCherry.rseg_run)
  } else {
    BigCherry.rseg_runs <- rbind(BigCherry.rseg_runs, BigCherry.rseg_run)
  }
}
  

#LoadStatsTable
BigCherry.rseg_summary <- data.frame()
#i <- 1
  for (i in 1:length(runid.list)){
    runid.i <- runid.list[i]
    run.i <- sub("runid_", "", runid.i)

    rseg.info.i <- fn_get_runfile_info(BigCherry.rseg.elid,run.i,site=omsite)
    rseg.metrics.i <- data.frame('model_version' = c('vahydro-1.0'),'runid' = c(runid.i),'runlabel' = BigCherry.rseg.metric.list,'metric' = BigCherry.rseg.metric.list)
    rseg_summary.i <- om_vahydro_metric_grid(metric, rseg.metrics.i,base_url = paste(site,"/entity-model-prop-level-export",sep=""))
    rseg_summary.i <- sqldf(paste("SELECT * FROM 'rseg_summary.i' WHERE featureid = ",BigCherry.rseg.hydroid,sep=""))

    if (nrow(rseg_summary.i) > 0) {
      rseg_summary.i <- cbind("runid" = run.i,"run_date" = rseg.info.i$run_date,
                              "starttime" = str_remove(rseg.info.i$starttime," 00:00:00"),
                              "endtime" = str_remove(rseg.info.i$endtime," 00:00:00"),rseg_summary.i)
      BigCherry.rseg_summary <- rbind(BigCherry.rseg_summary,rseg_summary.i)
    }
  }
BigCherry.rseg_summary <- BigCherry.rseg_summary %>% mutate_if(is.numeric, round, digits=2)
BigCherry.rseg_summary.T <- as.data.frame(t(BigCherry.rseg_summary[,-1]))
colnames(BigCherry.rseg_summary.T) <- BigCherry.rseg_summary[,1]
BigCherry_statsdf <- BigCherry.rseg_summary.T
BigCherry_statsdf <- cbind(rownames(BigCherry_statsdf),BigCherry_statsdf)
names(BigCherry_statsdf)[names(BigCherry_statsdf) == 'rownames(BigCherry_statsdf)'] <- 'Desc'  

BigCherry_sql <- paste('SELECT
                  CASE
                    WHEN "Desc" = "propname" THEN "Model Name"
                    WHEN "Desc" = "remaining_days_p0" THEN "Minimum Days of Storage Remaining"
                    ELSE Desc
                  END AS Description, *
                 FROM BigCherry_statsdf
                 WHERE Desc NOT IN ("riverseg","run_date","starttime","endtime","hydrocode","pid","featureid")
                 ',sep='')
BigCherrydf <- sqldf(BigCherry_sql)
BigCherrydf <- BigCherrydf[,-2]
```
...
if (!is.null(demand_url)) {
    cat("\n\n## Monthly Demand: ", short_names[[paste("runid_",intake_stats_runid,sep="")]],"\n\n")
...
Two scenarios are presented below to examine the alternatives for this permit re-issuance.  A summary of how permit rules affect available water for this permit, and how this operation may impact instream beneficial uses, and other downstream water withdrawals is presented.   
...
The model scenarios considered provide options to maximize safe yield, explore the potential for a more naturally varying hydrograph, and maintain adequate emergency storage in the event that future droughts are more severe than historical. Ultimately, the multi-year drought of 1998-2002, combined with the need to have a minimum of 60 days of storage remaining, will require a reduction in safe yield from the original 3.2 mgd. In order to meet the 60 days storage remaining, a 0.5mgd static flowby will probably supply a maximum demand of 2.6-2.8 mgd, comparable to that afforded by the 40% flowby scenario. 

With the current permit operations and a limit of 3.2 mgd they likely wouldn't be able to meet a 90% flowby and retain 60 days remaining storage in the reservoir during the drought of record. However a flowby closer to 40% combined with a reduced permit limit would likely be effective at maintaining storage levels and ensuring they can sustainably meet demand during dry periods while still better preserving the natural flow regime over a static 0.5 mgd flowby approach. This project would be unable to meet a 90% flowby without reducing demands from 3.2 mgd to below 1 mgd. Including drought triggers in the permit may help maintain storage levels in the reservoir during dry periods (this permit doesn't currently have drought triggers in place). Additionally, this facility has emergency connections with neighboring towns which may be sufficient to maintain supply during times of extreme drought. Note that demand is projected to decline according to water supply plan.

Overall, the scenarios examined support the option for moving towards a "percent of flow" flowby approach. It is important to note that modeling with the Big Cherry Dam maximum release capacity set to 15 cfs benefits reservoir storage by minimizing reservoir refill time, promoting improved storage recovery following dry periods. This 15 cfs, or some similarly modest maximum release, is crucial in enabling a PoF management schema to be effective. 
...
```{r BigCherryStatsTable, echo=FALSE}
BigCherry.ft <- qflextable(BigCherrydf)

# Set column widths
BigCherry.ft <- width(BigCherry.ft, j = 2:length(BigCherrydf[1,]), width = 1.9)
BigCherry.ft <- width(BigCherry.ft, j = 1, width = 2)
BigCherry.ft <- theme_box(BigCherry.ft)
BigCherry.ft <- bg(BigCherry.ft, bg = "#EFEFEF", part = "header")
fontsize(BigCherry.ft, size = 9)
```
...
for (i in 1:length(runid.list)){
  ...
  rseg_prop <- om_get_prop(site,BigCherry.rseg_runs[i,]$pid, "dh_properties", "fig.imp_storage.all")
  run_info <- find_name(fac_model_info,runid.i)
