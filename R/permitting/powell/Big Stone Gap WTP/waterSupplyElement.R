...
save_directory <- "C:/Users/nrf46657/Desktop/VWP Modeling/Big Stone Gap WTP/Sep_Oct2022_Coordination/big_cherry_5"
library(hydrotools)
# authenticate
...
pid <- 4826467 # BIG STONE GAP WTP:Powell River model 
elid <- 247415 
# runid <- 400
runid <- 600
...
# JK 10/12/22
###############################################################
# class(yrdatdf$unmet_demand_mgd)
# yrdatdf$unmet_demand_mgd < 1

yrdatdf <- sqldf("SELECT *, CASE
                         WHEN unmet_demand_mgd < 0.1 THEN 0.00
                             ELSE unmet_demand_mgd
                         END AS unmet_demand_mgd
               FROM yrdatdf
               ORDER BY year")

# remove first unmet_demand_mgd column
yrdatdf <- yrdatdf[,-min(which(names(yrdatdf) == "unmet_demand_mgd"))]


# yrmodat <- yrmodat[,-3] #remove the original avg_unmet col
###############################################################
###############################################################
...
# class(yrmodat)
# class(yrmodat$avg_unmet)

# options(scipen = 50) #get rid of scientific notation in the dataframe
# options(scipen = 0) # This is the default

# yrmodat <- sqldf("SELECT *, CASE
#                          WHEN avg_unmet < 0.09 THEN 0.00
#                              ELSE avg_unmet
#                          END AS avg_unmet
#                FROM yrmodat 
#                ORDER BY year")
