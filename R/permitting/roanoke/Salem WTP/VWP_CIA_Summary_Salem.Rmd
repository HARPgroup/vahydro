...
author: ""
title: "VWP CIA Summary - Salem WTP"
output: 
...
fac.name <- gsub("\\:.*","",fac.model$propname) #extract facility name from fac:rseg model name
...
The City of Salem owns and operates a public water system serving over 9,300 mixed use connections throughout the city. Raw water is treated at a single 10.0 MGD conventional water treatment plant located at 1300 Tidewater Street in the city limits. Salem utilizes a conjunctive use system comprised of a single 8 MGD intake in the Roanoke River in addition to three well sources with a combined capacity of 2.6 MGD.  

The applicant is applying for reissuance of a permit for an existing withdrawal on the Roanoke River with no proposed increase in the currently permitted volume of 8 million gallons per day (MGD).  

Permit: Salem City Water Treatment Plant, 98-1992  
Permit Dates: 2000-07-18 to 2015-07-17  

* **Daily Withdrawal Limit** = 8 mgd
* **Annual Withdrawal Limit** = 2920 mgy
    + (No annual limit defined in permit. Calculated by daily 8 mgd x 365 days = 2920 mgy)
* **Flow-by** = XXX
<br>
<br>
<br>
...
for (i in 1:length(runid.list)){
  ...
  fig_prop <- invisible(find_name(fac_run, "fig.unmet_heatmap_amt_gw"))
  if (!(is.null(fig_prop))) {
    fig_path <- fig_prop$code
    # we have an image, show it 
    cat("\n\n#### Heatmap: ",short_names[[runid.i]],"\n(Utilizing gw to offset unmet demand)\n")
    cat(paste0("![](",fig_path,")"),"\n")
  }
...
base_var <- fdc_vars[1] #BASE VARIABLE USED IN FDCs AND HYDROGRAPHS
comp_var <- fdc_vars[2] #VARIABLE TO COMPARE AGAINST BASE VARIABLE, TYPICALLY Qout
# base_var <- "Qnatural"
# comp_var <- "Qout"
########################################################## 
batch_list <- list()
for (i in 1:length(runid.list)) {
  ...
  #--------------------------------------------------    
    run_info.fac <- find_name(fac_model_info,runid_i)
  ...
  batch_list_i <- list(list(runid = runid_i,title_text = scenario_short_name.i))
  batch_list <- append(batch_list, batch_list_i)
}
########################################################## 
legend_text = c(paste("Flow (",base_var,")",sep=""),paste("Flow (",comp_var,")",sep=""))
#i <- 3

for (i in 1:length(batch_list)) {
  runid_i <- batch_list[[i]]$runid
  title_text_i <- batch_list[[i]]$title_text
  ...
  fac_df <- fac_df[ , !(names(fac_df) %in% "Qnatural")]#remove Qnatural col from Fac dataset if present
  ...
  ########################################################################################
  if (is.null(dat_join$Qnatural) == FALSE) {
    # FLOW DURATION CURVE PLOTS
    fdc_plot <- hydroTSM::fdc(
                            cbind(dat_join[names(dat_join)== base_var], dat_join[names(dat_join)== comp_var]),
    ...
    ymx <- 1000
    par(mar = c(5,5,2,5))
    hydrograph_dry <- plot(as.numeric(unlist(dat_join[names(dat_join)== base_var]))~as.Date(dat_join$date),
    ...
    par(new = TRUE)
    plot(as.numeric(unlist(dat_join[names(dat_join)== comp_var]))~as.Date(dat_join$date),type = "l",col='brown3', lwd = 2, 
          axes=FALSE,ylim=c(ymn,ymx),xlim=c(xmn,xmx),ylab="",xlab="")
    legend("topright",legend=legend_text,col=c("black","brown3"), lty=c(2,1), lwd=c(1,2), cex=1)
    ...
    ymx <- 5000
    par(mar = c(5,5,2,5))
    hydrograph_wet <- plot(as.numeric(unlist(dat_join[names(dat_join)== base_var]))~as.Date(dat_join$date),
                              type = "l", lty=2, lwd = 1,ylim=c(ymn,ymx),xlim=c(xmn,xmx),
                              ylab="Flow (cfs)",xlab=paste("\nDate\n",xmn,"to",xmx),
    ...
    par(new = TRUE)
    plot(as.numeric(unlist(dat_join[names(dat_join)== comp_var])),type = "l",col='brown3', lwd = 2,
          axes=FALSE,ylim=c(ymn,ymx),xlim=c(xmn,xmx),ylab="",xlab="")
    legend("topright",legend=legend_text,col=c("black","brown3"),lty=c(2,1), lwd=c(1,2), cex=1)
    print(hydrograph_wet)
  } #skip making plot if theres no Qnatural on the reseg model for a given runid
} #CLOSE FOR LOOP
...
if (isFALSE(upstream_rseg_ids) == FALSE & length(upstream_rseg_ids) > 0 & isFALSE(downstream_rseg_ids) == FALSE & length(downstream_rseg_ids) > 0) {  
    ...
    users_dn_df <- read.csv(paste(site,"/contains-mps-model-summary-export/",gsub(" ","",toString(downstream_rseg_ids)),sep=""))
    users_dn_df <- cbind(users_dn_df,Location="Downstream")
    users_df <- rbind(users_up_df,users_dn_df)
...
users_sql <- paste('SELECT usersdf.Location AS "Location", 
...
fac.rows <- which(usersdf$`Facility Name` == fac.name)
```
