...
# JK 7.18.23
# argst <- commandArgs(trailingOnly = T)
# river_seg <- "SmithMountainLake"
scenario_name <- 400
hydr_file_path <- "http://deq1.bse.vt.edu:81/data/proj3/out/runlog400.252119.log"
# model_version <- argst[4]
# rseg_ftype <- argst[5]
image_dir <- "C:/Users/nrf46657/Desktop/GitHub/vahydro/R/permitting/smith_mountain_lake"
save_url <- image_dir
# json_dir <- argst[7] #include file at the end!
###########################################################
...
sdate <- as.Date(min(hydr$thisdate)) #the right time span has been set in a previous script already
edate <- as.Date(max(hydr$thisdate))

# hydr <- zoo(hydr, order.by = hydr$index)
hydr <- zoo(hydr, order.by = hydr$thisdate)
...
# need to calcuate the l90 year manually since there's no json file for this model
Qin_g2 <- data.frame(group2(hydr$Qout))
l90 <- Qin_g2["X90.Day.Min"];
ndx = which.min(as.numeric(l90[,"X90.Day.Min"]));
l90_Qout = round(Qin_g2[ndx,]$"X90.Day.Min",6);
l90_year = Qin_g2[ndx,]$"year";
...
if (imp_off == 0) {
  ...
  if("impoundment" %in% cols) {
    ...
    hydroTSM::fdc(cbind(hydrpd$impoundment_Qin, hydrpd$impoundment_Qout),ylab="Q (cfs)",yat = c(0,100,1000,10000,max(cbind(hydrpd$impoundment_Qin, hydrpd$impoundment_Qout))),)
    dev.off()
    print(paste("Saved file: ", fname, "with URL", furl))
  ...
  if("Leesville_Lake" %in% cols) {
    river_seg <- "LeesvilleLake"
    # Plot and analyze impoundment sub-comps
    hydr$storage_pct <- hydr$Leesville_Lake_use_remain_mg * 3.07 / hydr$Leesville_Lake_max_usable
    #
    storage_pct <- mean(as.numeric(hydr$storage_pct) )
    if (is.na(storage_pct)) {
      usable_pct_p0 <- 0
    ...
    Leesville_Lake_days_remaining <- mean(as.numeric(hydr$Leesville_Lake_days_remaining) )
    if (is.na(Leesville_Lake_days_remaining)) {
      remaining_days_p0 <- 0
      remaining_days_p10 <- 0
      remaining_days_p50 <- 0
    } else {
      remaining_days = quantile(as.numeric(hydr$Leesville_Lake_days_remaining), c(0,0.1,0.5) )
      remaining_days_p0 <- remaining_days["0%"]
      remaining_days_p10 <- remaining_days["10%"]
      remaining_days_p50 <- remaining_days["50%"]
    }
    ...
    # vahydro_post_metric_to_scenprop(model_scenario$pid, 'om_class_Constant', NULL, 'remaining_days_p50', remaining_days_p50, ds)
    # 
    # this has an Leesville_Lake.  Plot it up.
    # Now zoom in on critical drought period
    pdstart = as.Date(paste0(l90_year,"-06-01") )
    pdend = as.Date(paste0(l90_year, "-11-15") )
    ...
    par(new = TRUE)
    plot(hydrpd$Leesville_Lake_Qin,col='blue', axes=FALSE, xlab="", ylab="")
    lines(hydrpd$Leesville_Lake_Qout,col='green')
    lines(hydrpd$Leesville_Lake_demand * 1.547,col='red')
    axis(side = 4)
    mtext(side = 4, line = 3, 'Flow/Demand (cfs)')
    legend("bottom",inset=-0.36, xpd=TRUE, c("Reservoir Storage","Inflow","Outflow","Demand"),
           col = c("black", "blue", "green","red"),
    ...
    png(fname)
    # hydroTSM::fdc(cbind(hydrpd$Leesville_Lake_Qin, hydrpd$Leesville_Lake_Qout),ylab="Q (cfs)")
    hydroTSM::fdc(cbind(hydrpd$Leesville_Lake_Qin, hydrpd$Leesville_Lake_Qout),ylab="Q (cfs)",yat = c(0,100,1000,10000,max(cbind(hydrpd$Leesville_Lake_Qin, hydrpd$Leesville_Lake_Qout))),)
    dev.off()
    print(paste("Saved file: ", fname, "with URL", furl))
    # vahydro_post_metric_to_scenprop(model_scenario$pid, 'dh_image_file', furl, 'fig.fdc.all.', 0.0, ds)
