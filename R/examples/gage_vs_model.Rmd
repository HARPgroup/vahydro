...
elid <- params$elid
runid <- params$runid
model_flow_col <- params$model_flow_col
...
gage_info <- dataRetrieval::readNWISsite(gageid)
...
barplot(
  ...
  col=c("blue", "black"),
  main=paste(gage_name, " USGS vs VAHydro, 90 Day Low Flow ",title_param,sep=""),
  beside=TRUE,
  ylab = "Streamflow (cfs)",
  ylim = c(0,ymax_val),
  cex.main=0.8,
...
usgs_fx <- qflextable(om_flow_table(historic, 'X_00060_00003'))
...
lt_plot <- plot(lt_mean_flows$model ~ lt_mean_flows$usgs, ylim=c(0, gage_info$drain_area_va))
lt_low_flows <- sqldf(paste("select * from both_flows where usgs <", 0.1 * gage_info$drain_area_va))
ltl_plot <- plot(lt_low_flows$model ~ lt_low_flows$usgs, ylim=c(0, 0.1 * gage_info$drain_area_va))
######################
```
...
if (nrow(crit_pd_data) > 0) {
  ...
  barplot(
    ...
    col=c("blue", "black"),
    main=paste("USGS vs VAHydro, Critical Low Flow Period",title_param, crit_pd_start, crit_pd_end,sep=""),
    beside=TRUE,
    ylab = "Streamflow (cfs)",
    ylim = c(0,ymax_val),
    cex.main=0.8,
