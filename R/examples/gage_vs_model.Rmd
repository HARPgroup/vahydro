...
if (!is.logical(model_output_file)) {
  ...
  model_flows <- zoo(as.numeric(as.character( model_data$Qout )), order.by = make_datetime(model_data$year, model_data$month, model_data$day, as.integer(model_data$hour)) )
} else {
  # standard OM output
...
barplot(
  ...
  col=c("blue", "black"),
  main=paste("USGS vs VAHydro, Qmean ",title_param,sep=""),
  beside=TRUE,
  ylab = "Streamflow (cfs)",
  ylim = c(0,ymax_val),
  cex.main=0.8,
