...
#runid.list <- c('runid_201','runid_401','runid_6011')
...
```{r loadPLOTS, echo=FALSE}
rlist <- gsub('runid_', '', runid.list)
figs.list <- paste0(site.plots,"/data/proj3/out/fig.imp_storage.all.",rseg.elid,".", rlist, ".png")

localpath = tempdir()
fig_paths <- character()
for(p in 1:length(figs.list)){
    PNG_item <- figs.list[p]
    PNG_filename <- paste0("fig.imp_storage.all.",rseg.elid,".", rlist[p], ".png")
    fig_paths <- append(fig_paths, paste(localpath,PNG_filename,sep="\\"))
    if (file.exists(paste(localpath, PNG_filename, sep = '/')) == FALSE) {
      destfile <- paste(localpath,PNG_filename,sep="\\")
      download.file(PNG_item, destfile = destfile, method = "curl")
    }
}      
``` 


```{r displayPLOTS, echo=FALSE, results = "asis"}
