...
source("https://raw.githubusercontent.com/HARPgroup/hydro-tools/master/R/om_model_table.R") #Used during development
...
fac_decimals = params$fac_decimals
rseg_decimals = params$rseg_decimals
...
if (exists("json_obj_url")) {
  ...
  if (!is.logical(impoundment_fid)) {
    ...
    imp_model_info <- ds$auth_read(imp_obj_url,  "text/json", "")
...
#Check database to find location map, will return a blank property if no map was
#generated and kept in database
fig_prop <- RomProperty$new(
  ds,
  list(propname = "fig.location_map",
       featureid = fac.model$pid),
  TRUE)
```

```{r LocationMap, echo=FALSE, results = "asis"}

if (!is.null(fig_prop) & !(is.logical(fig_prop))) {
...
#Current workaround for finfo bug in https://github.com/HARPgroup/hydro-tools/issues/381#issuecomment-1858190927
# finfo <- fn_get_runfile_info(elid, runid, scenid, site=omsite)
...
intake_sum_current <- om_flow_table(dat_current, intake_stats_varname, rdigits = cu_decimals)
ex_col = "10%"
...
#Round Table Values (Following McGurk request: "The tabulated values should be
#rounded to the nearest whole number.  The flow estimates at the stream gages
#are not precise or accurate enough for digits to the right of the decimal, and
#are not reported that way.  Therefore, modeled values should not be considered
#more accurate.")
# I allowed this to be 1 decimal place because when you have small watersheds,
# you can end up with a table of flows that are half populated with zeros. This
# results in the table being LESS accurate IMO than rounding to 1 place.  FWIW,
# the decimal places are not accuracy, they're precision so I'd argue that the
# model can be more precise cause it's a numerical calculation and therefore
# more precise :)

round_intake_sum_current <- intake_sum_current[2:length(intake_sum_current)]
round_intake_sum_current <- round(round_intake_sum_current,cu_decimals)
...
run_info <- find_name(fac_model_info,runid.i)
if (is.null(run_info$reports)) {
  cat(paste("*", paste0("__",run_names[runid.i],"__"),"-","Proposed scenario, ", runid.i, ", conclusions not provided.","\n", "Conclusions should be stored in [model] -> ", run.i, " -> reports -> conclusions"))
} else {
  cat(
    paste(
      "*", 
...
run.i <- sub("runid_", "", runid.i)
if (cu_model == "river") {
  # use the river as the entity to compare
  cu_elid = rseg.elid
} else {
  if (cu_model == "facility") {
    # use the facility
    cu_elid = fac.elid
  } else {
    # try this attribute as a elementid
    cu_elid = cu_model
  }
}
pr_data <- om_get_rundata(cu_elid, run.i, omsite, FALSE)
...
if (!is.logical(cu_base_runid)) {
  base_data <- as.data.frame(om_get_rundata(cu_elid, cu_base_runid, omsite, FALSE))
...
fqcu_table <- om_cu_table(fac_report_info, pr_data_cmp, cu_post_var, use_pre_var, cu_threshold, cu_decimals) 
fqcu_caption <- paste(
  ...
  fac_report_info$intake_name$value, 
  " and all upstream point-source flows.  ", 
...
### Consumptive Use Table for Proposed Scenario

```{r LoadConclusionsFigs, echo=FALSE, results = "asis"}

cat(fqcu_table$caption_text)
fqcu_table
  

if (!is.null(demand_url)) {
  cat("\n")
  cat("### Demand Chart for Preferred Scenario")
  cat("\n")
...
fac_table_sql <- paste('SELECT
                  CASE
                    WHEN "Desc" = "model" THEN "Facility Model Statistics:"
                    WHEN "Desc" = "unmet30_mgd" THEN "Maximum 30 day conservation/unmet demand (MGD)"
                    WHEN "Desc" = "base_demand_mgy" THEN "Base Demand (MGY)"
                    WHEN "Desc" = "wd_mgy" THEN "Withdrawal (MGY)"
                    WHEN "Desc" = "unmet_demand_mgy" THEN "Conservation/Unmet Demand (MGY)"
...
for (i in 1:length(runid.list)){
  if (!is.logical(impoundment_fid)) {
    ...
    if (!(is.null(fig_prop))) {
      ...
      fig_md = paste(fig_md, paste0("![](",fig_path,")"),"\n")
      fig_md = paste(fig_md, "\n\n\\pagebreak\n")
    } else {
      # tbd: try impoundment linked to river segment.
      fig_md = paste(fig_md, paste("No active impoundment found for run id",runid.i,"on impoundment feature", impoundment_fid)) 
    }
  } else {
    # look for impoundment on river model run prop 
    runid.i <- runid.list[i]
    ...
    if (!(is.null(fig_prop))) {
      ...
      # tbd: try impoundment linked to river segment.
      fig_md = paste(fig_md, paste("No active impoundment found for run id",runid.i))
    }
  }
}
...
if (has_imp) {
  cat("### Analysis of Reservoir Storage: \n\n
...
```{r makeCUplots, echo=FALSE, include=FALSE}
cu_tables = list()
# check rseg model for plots
for (i in 1:length(runid.list)){
  # get river model run prop 
  runid.i <- runid.list[i]
  # generate a CU table for each run
  run.i <- sub("runid_", "", runid.i)
  ...
  fqcu_caption <- paste(
    "Modeled monthly consumptive use statistics in the ", 
    rseg_feature$name, "in cubic feet per second (cfs). Columns show the modeled non-exceedance flow percentiles and the consumptive use % due to cumulative demands for", 
    fac_run$reports$scenario_name$value, 
    ". Simulated demands include demands at ", 
    fac_report_info$intake_name$value, 
    ", including all up-stream demands and all upstream point-source flows.  ", 
    "Fields that are marked as 'n/a' indicate that the baseline flow for that time period/percentile was below the model accuracy threshold of ", cu_min_valid,"cfs.",
    base_msg
  )
  fqcu_table$caption_text <- fqcu_caption
  #fqcu_table
  cu_tables[[i]] = fqcu_table
  #cat("\n")
  #cat("#### CU Table for")
  #fqcu_table
  #cat("\n")
...
### Cumulative Consumptive Use Plots:


```{r LoadCUPlots, echo=FALSE, results = "asis"}
for (i in 1:length(runid.list)){
  cat(paste("#### Cumulative use for", run_names[runid.list[i]],"\n"))
  cat(cu_tables[[i]]$caption_text)
  cat("\n")
  #cutab = format(cu_tables[[i]], type = "docx")
  cutab = cu_tables[[i]]
  cat(flextable::flextable_to_rmd(cutab))
  #cutab <- c(
  #  "before",
  #  knitr::knit_print(cutab),
  #  "  \\newline\n"
  #)
  #knitr::asis_output(paste(cutab, collapse = "\n"))
  cat("\n\n\\pagebreak\n")
  #break
}
...
### Analysis of Potential Conservation/Unmet Demand at the River Intake:
The following grids are data plotting tools that help visualize data as magnitudes of color intensity. These depict the number of days with required conservation demand reductions or unmet demands for each month of the simulation (due to drought triggers or demands exceeding allowable withdrawal at the intake based on the cumulative conditions in the watershed and the flow-by rules in effect). The cells show the amount of reductions/unmet demand for each month [Number of Unmet Days & Amount (MGD)]. Hydrographs are shown for the period of the simulation with greatest reduction/unmet demand.
...
for (i in 1:length(runid.list)){
  ...
  if (!(is.null(fig_prop))) {
    fig_path <- fig_prop$code
    # we have an image, show it 
    cat("\n\n#### Drought reduction/Unmet Demand: ",short_names[[runid.i]],"\n\n")
    cat(paste0("![](",fig_path,")"),"\n")
  }
...
for (i in 1:length(runid.list)){
  ...
  runid.i <- runid.list[i]
  if (!is.logical(impoundment_fid)) {
    # impoundment specified, use imp fdc and drought plots 
    # no imppoundment, use river seg FDC and drought 
    imp_run <- find_name(imp_model_info, runid.i)
    fig_prop <- invisible(find_name(imp_run, "fig.fdc.all."))
    if (!(is.null(fig_prop))) {
      fig_path <- fig_prop$code
      # we have an image, show it 
      cat("\n\n#### ",short_names[[runid.i]],":","\n\n")
...
- Maximum 30 day potential drought reduction/Unmet Demand (MGD): The largest difference between *Requested Demand* and *Withdrawal Met* that results during a continuous 30-day simulation period.
...
- Drought Reduction/Unmet Demand: The difference between *Base Demand* and *Withdrawal Met*, on average, throughout the entire simulation period.
...
if (elfgen) {
  run_container <- RomProperty$new(
  ds,
  list(propname = runid.list[1],
       entity_type = "dh_properties",
       featureid = rseg.model$pid
       ),
  TRUE)
  
  fig_path = FALSE
  elfgen_EDAS_huc8 <- RomProperty$new(
    ds,
    list(propname = "elfgen_EDAS_huc8",
         entity_type = "dh_properties",
         featureid = run_container$pid
    ),
    TRUE)
  
  if (!is.logical(elfgen_EDAS_huc8)) {
    fig.elfgen <- RomProperty$new(
      ds,
      list(propname = "fig.elfgen",
           entity_type = "dh_properties",
           featureid = elfgen_EDAS_huc8$pid
      ),
      TRUE)
...
if (elfgen) {
  if (!is.logical(fig_path)) {
    ...
    # cat("\newpage")
    cat("Estimates for richness change are presented both as an absolute number of species (Richness Change (abs)) and as a percentage of the total number of species present (Richness Change (%)). Richness change calculations are derived from the estimated percent total consumptive use^[Kleiner et al: [https://onlinelibrary.wiley.com/doi/full/10.1111/1752-1688.12876](https://onlinelibrary.wiley.com/doi/full/10.1111/1752-1688.12876) & Rapp et al: [https://onlinelibrary.wiley.com/doi/full/10.1111/1752-1688.12877](https://onlinelibrary.wiley.com/doi/full/10.1111/1752-1688.12877)]. Note: elfgen methodology only applicable for watersheds < 800 cfs mean annual flow.")
    elfgen_df
  } else {
...
'"FROM users_df AS usersdf
  ...
  WHERE usersdf.mp_bundle = "intake" AND usersdf.mp_fstatus = "active"
