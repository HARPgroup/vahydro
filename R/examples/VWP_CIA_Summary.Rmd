...
if (cu_model == "river") {
  ...
  if (cu_model == "facility") {
    # use the facility
    pr_data <- om_get_rundata(fac.elid, run.i, omsite, FALSE)
  } else {
    # try this attribute as a elementid
    pr_data <- om_get_rundata(cu_model, run.i, omsite, FALSE)
  }
}
...
for (i in 1:length(runid.list)){
  ...
  fqcu_caption <- paste(
    ...
    fac_run_info$reports$scenario_name$value, 
    ". Simulated demands include all up-stream demands and demands at ", 
...
for (i in 1:length(runid.list)){
  ...
  cutab = cu_tables[[i]]
  flextable::flextable_to_rmd(cutab)
  #cutab <- c(
  #  "before",
