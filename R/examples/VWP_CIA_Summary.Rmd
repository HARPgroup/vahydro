...
json_obj_url <- paste(json_obj_url, fac.model$pid, sep="/")
fac_model_info <- om_auth_read(json_obj_url, token,  "text/json", "")
...
for (i in 1:length(runid.list)){
  ...
  if (is.null(run_info$reports)) {
    print(paste("*", run_info$name,"-","Run report information not provided."))
  } else {
    print(paste("*", run_info$reports$scenario_name$value,"-",run_info$reports$scenario_intro$value))
  }
