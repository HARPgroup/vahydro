...
for (i in 1:length(runid.list)){
  ...
  short_names[[runid.i]] = as.character(find_name(fac_run_info,'reports')$scenario_short_name$value)
  if (is.null(short_names[[runid.i]])) {
    short_names[[runid.i]] <- runid.i
  }
  run_names[runid.i] = as.character(find_name(fac_run_info,'reports')$scenario_name$value)
  if (is.null(run_names[runid.i])) {
