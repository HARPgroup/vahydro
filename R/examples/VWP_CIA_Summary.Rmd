...
for (i in 1:length(runid.list)){
  ...
  fqcu_caption <- paste(
    ...
    fac_model_info$reports$scenario_name$value, 
    ". Simulated demands include all up-stream demands and demands at ", 
...
for (i in 1:length(runid.list)){
  ...
  if (!(is.null(fig_prop))) {
    fig_path <- fig_prop$code
    # we have an image, show it 
    cat("\n\n#### Drought reduction/: ",short_names[[runid.i]],"\n\n")
    cat(paste0("![](",fig_path,")"),"\n")
  }
...
- Maximum 30 day potential drought reduction/ (MGD): The largest difference between *Requested Demand* and *Withdrawal Met* that results during a continuous 30-day simulation period.
...
- Drought Reduction/: The difference between *Base Demand* and *Withdrawal Met*, on average, throughout the entire simulation period.
