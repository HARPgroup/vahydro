...
###dat_current <- om_get_rundata(fac.elid, 11, omsite, FALSE)
###intake_sum_current <- om_flow_table(dat_current, "Qintake")
```
...
for (i in 1:length(runid.list)){
  ...
  run_info <- find_name(fac_model_info,runid.i)
  if (is.null(run_info$reports)) {
    cat(paste("*", paste0("__",run_info$name,"__"),"-","Run analysis not provided.","\n"))
  } else {
    ri <- run_info$reports
    cat(
      paste(
...
statsdf_sql <- paste('SELECT
                  CASE
                    WHEN "Desc" = "riverseg" THEN "River Segment"
                  ...
                  END AS Description, *
                 FROM statsdf
                 WHERE Desc NOT IN ("run_date","starttime","endtime")
                 ',sep='')
...
#--------------------------------------------------------------------------
#BigCherry
# BigCherry.statsdf <- om_cia_table(
#   rseg.hydroid = BigCherry.rseg.hydroid,
#   fac.hydroid = fac.hydroid,
#   runid.list = runid.list, 
#   fac.metric.list = c('unmet30_mgd'),
#   rseg.metric.list = BigCherry.rseg.metric.list,
#   site,
#   omsite
# )
...
BigCherry_sql <- paste('SELECT
                  CASE
                    WHEN "Desc" = "propname" THEN "River Segment"
                    WHEN "Desc" = "remaining_days_p0" THEN "Minimum Days of Storage Remaining"
                    ELSE Desc
                  END AS Description, *
...
# Set column widths
BigCherry.ft <- width(BigCherry.ft, j = 2:length(BigCherrydf[1,]), width = 1.5)
