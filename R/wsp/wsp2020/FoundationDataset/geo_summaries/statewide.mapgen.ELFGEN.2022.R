...
#library(wicket) #wkt_centroid()  #PACKAGE NOT AVAILABLE 2/14/22
...
WBDF <- read.table(file = 'C:/Users/nrf46657/Desktop/GitHub/HARParchive/GIS_layers/WBDF.csv', sep = ',', header = TRUE)
...
# RUN FUNCTION
######################################################################################################
metric <- "consumptive_use_frac" #- NOTE: I don't actually think this is used in the script. Referenced only once in script at LINE 31; RSeg_summary df not used at all.
runid_a <- "runid_11" #"runid_11","runid_13","runid_18"
richness_change <- "X2020_chg_pct" #"X2020_chg_pct", "X2040_chg_pct", "Exempt_chg_pct"
huc_level <- "huc8"
...
#elfgen_dataset <- read.csv(paste0("C:\\Users\\nrf46657\\Desktop\\elfgen_StatePlan_Data\\","elfgen_dataset_rseg_summary_",richness_chg,".csv"))
elfgen_dataset <- read.csv(paste0("C:\\Users\\nrf46657\\Desktop\\elfgen_StatePlan_Data\\","elfgen_dataset_rseg_summary_richness_change.csv"))
#colnames(elfgen_dataset)

plt <- statewide.mapgen.ELFGEN(metric,runid_a,richness_change,huc_level,richness_chg,elfgen_dataset)
###########################################################
...
statewide.mapgen.ELFGEN <- function(metric,runid_a,richness_change,huc_level,richness_chg,elfgen_dataset){
...
#   
  ...
  #COMMENTED OUT THIS WHOLE SECTION 2/14/22
  # # NEED TO REMOVE ANY WITH Rseg Outlet MAF > breakpt 
  # 
  # #ADDED THIS breakpt LINE IN ON 2/14/22
  # breakpt <- 530
  ...
  #COMMENTED OUT 2/14/22 - would need to track down those legend pngs
  ...
  #CHANGED THIS 2.14.22
  #richness_change <- "richness_change"
  #richness_change <- "X2020_chg_pct"
  richness_change <- richness_change
  #colnames(RSeg_data)
  ...
  #print("I MADE IT HERE - after geom6")
  ...
  #COMMENTED ELSE OUT ON 2/14/22
  #if ((length(RSeg_tidal[,1]) >= 1) == TRUE) {
    #RSeg_data_test <- RSeg_data[1,]
    #group_tidal_base <- st_as_sf(RSeg_data_test, wkt = 'geom')
    
    #group_tidal_base <- st_as_sf(RSeg_data, wkt = 'geom')
    group_tidal_base <- RSeg_data
  ...
  #print("I MADE IT HERE - after geom_tidal")
  ...
  #THIS WAS THE BEFORE 2/14/22
  #export_file <- paste0(export_path, "tables_maps/Xfigures/VA_",runid_a,"_",richness_chg,"_",huc_level,"_map.png",sep = "")
  export_file <- paste0("C:/Users/nrf46657/Desktop/elfgen_StatePlan_Data/Xfigures_2.14.22/VA_",runid_a,"_",richness_chg,"_",huc_level,"_map.png",sep = "")
