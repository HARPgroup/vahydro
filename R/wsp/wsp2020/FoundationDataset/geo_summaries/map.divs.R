...
map.divs.one <- function(RSeg_data,rseg_border,color_scale,divs,runid_a){
  ...
  if (nrow(bin1) > 0) {
    geom1 <- geom_sf(data = bin1,aes(geometry = geom,fill = 'antiquewhite',colour=rseg_border), inherit.aes = FALSE, show.legend = TRUE)
    color_values <- color_scale[1]
    label_values <- paste(" More than ",div1,"",sep="")
  } else  {
    geom1 <- geom_blank()
  ...
  if (nrow(bin2) > 0) {
    geom2 <- geom_sf(data = bin2,aes(geometry = geom,fill = 'antiquewhite1',colour=rseg_border), inherit.aes = FALSE, show.legend = TRUE)
    color_values <- rbind(color_values,color_scale[2])
    label_values <- rbind(label_values,paste(div1," to ",div2,"",sep=""))
  } else  {
    geom2 <- geom_blank()
  ...
  if (nrow(bin3) > 0) {
    geom3 <- geom_sf(data = bin3,aes(geometry = geom,fill = 'antiquewhite2',colour=rseg_border), inherit.aes = FALSE, show.legend = TRUE)
    color_values <- rbind(color_values,color_scale[3])
    label_values <- rbind(label_values,paste(div2," to ",div3,"",sep=""))
  } else  {
    geom3 <- geom_blank()
  ...
  if (nrow(bin4) > 0) {
    geom4 <- geom_sf(data = bin4,aes(geometry = geom,fill = 'antiquewhite3',colour=rseg_border), inherit.aes = FALSE, show.legend = TRUE)
    color_values <- rbind(color_values,color_scale[4])
