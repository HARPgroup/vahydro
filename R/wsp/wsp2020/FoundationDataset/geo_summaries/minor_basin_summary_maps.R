...
#  geom_sf(data = RSeg_data,aes(geometry = geom,fill = 'aliceblue'), inherit.aes = FALSE,  show.legend=FALSE)+ 
  ...
  #geom_sf(data = group_negInf_neg20,aes(geometry = geom,fill = 'antiquewhite4'), inherit.aes = FALSE)+ 
    ...
    scale_fill_manual(values=c("darkolivegreen3","cornflowerblue","khaki2","plum3"), 
                    name = "Legend",
                    labels = c(">= 0%", 
                               "-5% 50 0%", 
                               "-10% to -5%", 
...
ggsave(plot = map, file = paste0(folder, "state_plan_figures/single_basin/chg_11_to_13_",metric,"_",minorbasin,"_map.png"), width=6.5, height=5)
