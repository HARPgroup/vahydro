...
minorbasin.mapgen <- function(minorbasin,metric,runid_a,runid_b,wd_points = "OFF",rsegs = "ON"){
  ...
  partitions <- c(0.5,1.0,2.0,5.0,10,25,50)
  
  bins_sql <-  paste("SELECT *,
	                  CASE WHEN mp_2040_mgy/365.25 < 0.5 THEN '1'
		                WHEN mp_2040_mgy/365.25 >=  0.5 AND mp_2040_mgy/365.25 < 1.0 THEN '2'
		                WHEN mp_2040_mgy/365.25 >=  1.0 AND mp_2040_mgy/365.25 < 2.0 THEN '3'
		                WHEN mp_2040_mgy/365.25 >=  2.0 AND mp_2040_mgy/365.25 < 5.0 THEN '4'
		                WHEN mp_2040_mgy/365.25 >=  5.0 AND mp_2040_mgy/365.25 < 10 THEN '5'
		                WHEN mp_2040_mgy/365.25 >= 10 AND mp_2040_mgy/365.25 < 25 THEN '6'
		                WHEN mp_2040_mgy/365.25 >= 25 AND mp_2040_mgy/365.25 < 50 THEN '7'
		                ELSE '8'
		                END AS bin
		                FROM mp_layer",sep="")
