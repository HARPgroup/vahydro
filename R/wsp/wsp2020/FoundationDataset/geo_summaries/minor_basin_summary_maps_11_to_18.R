...
metric <- "l30_Qout"
plot_title <- "30 Day Low Flow (Percent Change 2020 to Exempt Users Run)"
...
#  geom_sf(data = RSeg_data,aes(geometry = geom,fill = 'aliceblue'), inherit.aes = FALSE,  show.legend=FALSE)+ 
  ...
  scale_fill_manual(values=c("darkolivegreen3","cornflowerblue","plum3","coral3"),
                    name = "Legend",
                    labels = c(">= 0%",
                               "-5% to 0%",
                               "-20% to -10%",
...
ggsave(plot = map, file = paste0(folder, "state_plan_figures/single_basin/chg_11_to_18_",metric,"_",minorbasin,"_map.png"), width=6.5, height=5)
