...
elid = 224287    #347363 #297938 # 236079  # 278660 #345486 #344054     	
runid = 18
...
yrdat <- sqldf(
  "select year, avg(wd_cumulative_mgd) as wd_cumulative_mgd, 
    round(avg(wd_mgd),2) as wd_mgd , 
    round(avg(ps_cumulative_mgd),2) as ps_cumulative_mgd, 
    round(avg(ps_mgd),2) as ps_mgd , 
...
yrmodat <- sqldf(
  "select year, month, avg(wd_cumulative_mgd) as wd_cumulative_mgd, 
    round(avg(wd_mgd),2) as wd_mgd , 
    round(avg(ps_cumulative_mgd),2) as ps_cumulative_mgd, 
    round(avg(ps_mgd),2) as ps_mgd , 
    round(avg(Qout),2) as Qout 
...
pddat <- sqldf(
  "select avg(wd_cumulative_mgd) as wd_cumulative_mgd, 
    round(avg(wd_mgd),2) as wd_mgd , 
    round(avg(ps_cumulative_mgd),2) as ps_cumulative_mgd, 
    round(avg(ps_mgd),2) as ps_mgd , 
    round(avg(Qout),2) as Qout 
