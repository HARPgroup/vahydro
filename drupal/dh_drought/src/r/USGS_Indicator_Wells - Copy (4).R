...
for (j in 1:length(hydrocodes)) {
  ...
  # #EXCLUDE EST measurements
  # data_est <-   paste("SELECT *
  #                             FROM data
  #                             WHERE tz_cd != 'EST'",sep="")
  # data <- sqldf(data_est)
  ...
  #------------------------------------------------------------
  # max_colname <- colnames(data)[5]
  # colnames(data)[7]
  #
  # which(colnames(data) == grep("^_72019_00001",colnames(data)) )
  ...
  #data <- sqldf(data_cols)
  data$year <- year(data[,"datetime"])

  
  # #EXCLUDE EMPTY MEDIANS, needed for days when no data was recorded
  #THIS IS NEEDED PRIOR TO CALCULATING ANNUAL median_depth, min_depth, max_depth STATISTICS
   data_blank <-   paste("SELECT *
                          FROM data
                          WHERE median != ''",sep="")
   data <- sqldf(data_blank)
  
  
  
  #(",max_colname,"+",min_colname,")/2

  # #lapply(data, class)
  # data$median <- as.character(data$median)
  ...
  #                       ",max_colname," AS max,
  #                       ",min_colname," AS min,
  #                       (",max_colname,"+",min_colname,")/2 AS median
  #                       FROM data
  #                       ",sep="")
  # data <- sqldf(data_cols)
  #
  ...
  #------------------------------------------------------------
  #------------------------------------------------------------
#   tester <- data.frame(year = data$year, median = data$median)
#   x2019 <- tester[which(tester$year == 2019),]
# min(as.numeric(x2019$median))
# median(as.numeric(x2019$median))
# max(as.numeric(x2019$median))
# 
